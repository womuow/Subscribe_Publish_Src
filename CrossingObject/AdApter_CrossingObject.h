...
class AdApter_CrossingObject
{
public:
	AdApter_CrossingObject();
	~AdApter_CrossingObject();	void run();
	std::string json_file = "";
	dds::pub::DataWriter<CrossingObjectModule::CrossingObject> topic_writer_init(std::string topicName);
	CrossingObjectModule::CrossingObject topic_out;
	std::string topicName = "dds2rte/CrossingObject";
	int domiain_id = 0;
	bool type = false;
