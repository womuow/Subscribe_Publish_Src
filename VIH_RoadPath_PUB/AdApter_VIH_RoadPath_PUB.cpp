#include"AdApter_VIH_RoadPath_PUB.h"



AdApter_VIH_RoadPathPUB::AdApter_VIH_RoadPathPUB()
{
}
AdApter_VIH_RoadPathPUB::~AdApter_VIH_RoadPathPUB()
{
}
auto AdApter_VIH_RoadPathPUB::maxeye_midware_init()
{
...
void AdApter_VIH_RoadPathPUB::run()
{
    ...
    RoadPath VIH_RoadPath_;
    setIntialValue_RoadPath(VIH_RoadPath_);

    while (true) {        
        data_in.resize(sizeof(VIH_RoadPath_));
        std::memcpy(&data_in[0], &VIH_RoadPath_, sizeof(VIH_RoadPath_));
...
int main()
{
    ...
    std::cout << "Running VIH_RoadPath_PUB on Linux"<< fullPath << std::endl;
#endif
    size_t pos = fullPath.find_last_of("\\/");
    std::string configPath = fullPath.substr(0, pos) + "/discovery_config.json";
    AdApter_VIH_RoadPathPUB objtest;
