...
class AdApter_LDPPath
{
public:
	AdApter_LDPPath();
	~AdApter_LDPPath();	void run();
	std::string json_file = "";
	dds::pub::DataWriter<LDPPathModule::LDPPath> topic_writer_init(std::string topicName);
	LDPPathModule::LDPPath topic_out;
	std::string topicName = "dds2rte/LDPPath";
	int domiain_id = 0;
	bool type = false;
