#include"AdApter_VIH_RoadInfo_PUB.h"



AdApter_VIH_RoadInfoPUB::AdApter_VIH_RoadInfoPUB()
{
}
AdApter_VIH_RoadInfoPUB::~AdApter_VIH_RoadInfoPUB()
{
}
auto AdApter_VIH_RoadInfoPUB::maxeye_midware_init()
{
...
void AdApter_VIH_RoadInfoPUB::run()
{
    int i=0;
    std::cout << "config file path: " << json_file.c_str() << std::endl;
    data_in.resize(sizeof(RoadInfo_T));
    MOS::communication::Init(json_file.c_str());
    MOS::utils::Register::get().register_version("libVIH_RoadInfoPUB", "1.1.0");
    ...
    RoadInfo_T VIH_RoadInfo_;
    setIntialValue_RoadInfo_T(VIH_RoadInfo_);

    while (true) {        
        data_in.resize(sizeof(VIH_RoadInfo_));
        std::memcpy(&data_in[0], &VIH_RoadInfo_, sizeof(VIH_RoadInfo_));
...
int main()
{
    ...
    std::cout << "Running VIH_RoadInfo_PUB on Linux"<< fullPath << std::endl;
#endif
    size_t pos = fullPath.find_last_of("\\/");
    std::string configPath = fullPath.substr(0, pos) + "/discovery_config.json";
    AdApter_VIH_RoadInfoPUB objtest;
