...
#ifndef ADAPTER_ROADEDGE_PUB_H
#define ADAPTER_ROADEDGE_PUB_H



using namespace std::chrono_literals;
// using namespace org::eclipse::cyclonedds;
class AdApter_RoadEdgePUB
{
public:
	AdApter_RoadEdgePUB();
	~AdApter_RoadEdgePUB();	void run();
	std::string json_file = "";
	// dds::sub::DataReader<RoadEdgePUBModule::RoadEdgePUB> topic_read_init(std::string topicName);
	// RoadEdgePUBModule::RoadEdgePUB topic_out;
	//std::string topicName = "rte2dds/RoadEdgePUB";
	int domiain_id = 0;
	bool type = false;
	std::string topic = "SF/RoadEdge";
	std::string data_in = "";
	...
	// void update_objInfo(const RoadEdgePUBModule::RoadEdgePUB msg);
private:
};



#endif // ADAPTER_ROADEDGE_PUB_H