...
class AdApter_RoadEdge
{
public:
	AdApter_RoadEdge();
	~AdApter_RoadEdge();	void run();
	std::string json_file = "";
	dds::pub::DataWriter<RoadEdgeModule::RoadEdge> topic_writer_init(std::string topicName);
	RoadEdgeModule::RoadEdge topic_out;
	std::string topicName = "dds2rte/RoadEdge";
	int domiain_id = 0;
	bool type = false;
