// File: CCITT-FALSE-16.cpp
#include "CCITT-FALSE-16.hpp"

// CCITT-FALSE parameter config
#define CRC16_POLYNOMIAL          0x1021U
#define CRC16_INITIAL_VALUE       0xFFFFU
#define CRC16_XOR_VALUE           0x0000U
#define CRC16_JUNK                8U     
#define CRC16_JUNKREST            8U     
#define CRC_TABLEMASK             0x00FFU
#define CRC_TABLESIZE             256U   

const uint16_t  crc16_table[CRC_TABLESIZE] =
{
                            ...
                            bool is_first_call)
{
    ...
    if (data_ptr != nullptr)
    {
        if (is_first_call)
        {
            crc_temp = CRC16_INITIAL_VALUE;
        } else {
            crc_temp = start_value ^ CRC16_XOR_VALUE;
        }
        
        for (index = 0U; index < data_len; ++index)
        {
            crc_temp ^= ((uint16_t)data_ptr[index]) << CRC16_JUNKREST;
            
            crc_temp = (crc_temp << CRC16_JUNK) ^ 
                        crc16_table[(crc_temp >> CRC16_JUNKREST) & CRC_TABLEMASK];
        }
        
        result = crc_temp ^ CRC16_XOR_VALUE;
    }
