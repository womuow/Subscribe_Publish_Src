...
#ifndef ADAPTER_LDP_OBJ_PUB_H
#define ADAPTER_LDP_OBJ_PUB_H

using namespace std::chrono_literals;
// using namespace org::eclipse::cyclonedds;
class AdApter_LDP_ObjPUB
{
public:
	AdApter_LDP_ObjPUB();
	~AdApter_LDP_ObjPUB();	void run();
	std::string json_file = "";
	// dds::sub::DataReader<LDP_ObjPUBModule::LDP_ObjPUB> topic_read_init(std::string topicName);
	// LDP_ObjPUBModule::LDP_ObjPUB topic_out;
	//std::string topicName = "rte2dds/LDP_ObjPUB";
	int domiain_id = 0;
	bool type = false;
	std::string topic = "ThrtAssemt/LDP_Obj";
	std::string data_in = "";
	...
	// void update_objInfo(const LDP_ObjPUBModule::LDP_ObjPUB msg);
private:
};



#endif // ADAPTER_LDP_OBJ_PUB_H