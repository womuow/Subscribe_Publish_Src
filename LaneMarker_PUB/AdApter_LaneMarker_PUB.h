...
#ifndef ADAPTER_LANEMARKER_PUB_H
#define ADAPTER_LANEMARKER_PUB_H
using namespace std::chrono_literals;
// using namespace org::eclipse::cyclonedds;
class AdApter_LaneMarkerPUB
{
public:
	AdApter_LaneMarkerPUB();
	~AdApter_LaneMarkerPUB();	void run();
	std::string json_file = "";
	// dds::sub::DataReader<LaneMarkerPUBModule::LaneMarkerPUB> topic_read_init(std::string topicName);
	// LaneMarkerPUBModule::LaneMarkerPUB topic_out;
	//std::string topicName = "rte2dds/LaneMarkerPUB";
	int domiain_id = 0;
	bool type = false;
	...
	// void update_objInfo(const LaneMarkerPUBModule::LaneMarkerPUB msg);
private:
};
