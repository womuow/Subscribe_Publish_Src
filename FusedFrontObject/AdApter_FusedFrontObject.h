...
#include <thread>
#include "dds/dds.hpp"
#include "FusedFrontObject.hpp"
#include "FusedFrontObject.h"
#include <atomic>
#include <chrono>
#include <csignal>
#include <filesystem>
...
class AdApter_FusedFrontObject
{
public:
	AdApter_FusedFrontObject();
	~AdApter_FusedFrontObject();	void run();
	std::string json_file = "";
	dds::pub::DataWriter<FusedFrontObjectModule::FusedFrontObject> topic_writer_init(std::string topicName);
	FusedFrontObjectModule::FusedFrontObject topic_out;
	std::string topicName = "dds2rte/FusedFrontObject";
	int domiain_id = 0;
	bool type = false;
