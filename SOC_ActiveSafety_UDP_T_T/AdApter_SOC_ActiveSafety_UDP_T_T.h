...
#include <thread>
#include "dds/dds.hpp"
#include "SOC_ActiveSafety_UDP_T_T.hpp"
#include "SOC_ActiveSafety_UDP_T_T.h"
#include <atomic>
#include <chrono>
#include <csignal>
#include <filesystem>
...
class AdApter_SOC_ActiveSafety_UDP_T_T
{
public:
	AdApter_SOC_ActiveSafety_UDP_T_T();
	~AdApter_SOC_ActiveSafety_UDP_T_T();	void run();
	std::string json_file = "";
	dds::pub::DataWriter<SOC_ActiveSafety_UDP_T_TModule::SOC_ActiveSafety_UDP_T_T> topic_writer_init(std::string topicName);
	SOC_ActiveSafety_UDP_T_TModule::SOC_ActiveSafety_UDP_T_T topic_out;
	std::string topicName = "dds2rte/SOC_ActiveSafety_UDP_T_T";
	int domiain_id = 0;
	bool type = false;
	std::string topic = "ACTIVESAFETY/activesafety";
	std::string data_in = "";
