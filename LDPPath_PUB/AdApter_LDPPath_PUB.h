...
class AdApter_LDPPathPUB
{
public:
	AdApter_LDPPathPUB();
	~AdApter_LDPPathPUB();	void run();
	std::string json_file = "";
	// dds::sub::DataReader<LDPPathPUBModule::LDPPathPUB> topic_read_init(std::string topicName);
	// LDPPathPUBModule::LDPPathPUB topic_out;
	//std::string topicName = "rte2dds/LDPPathPUB";
	int domiain_id = 0;
	bool type = false;
	std::string topic = "ThrtAssemt/LDPPath";
	std::string data_in = "";
	...
	// void update_objInfo(const LDPPathPUBModule::LDPPathPUB msg);
private:
};



#endif // ADAPTER_CAN_ACTIVESAFETY_PUB_H