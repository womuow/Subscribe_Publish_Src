...
#include <thread>
#include "dds/dds.hpp"
#include "RearObject.hpp"
#include "RearObject.h"
#include <atomic>
#include <chrono>
#include <csignal>
#include <filesystem>
...
class AdApter_RearObject
{
public:
	AdApter_RearObject();
	~AdApter_RearObject();	void run();
	std::string json_file = "";
	dds::pub::DataWriter<RearObjectModule::RearObject> topic_writer_init(std::string topicName);
	RearObjectModule::RearObject topic_out;
	std::string topicName = "dds2rte/RearObject";
	int domiain_id = 0;
	bool type = false;
